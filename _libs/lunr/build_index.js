...
function readHtml(root, file, fileId) {
    ...
    var published = $(".not_published").length ? false : true;
...
function buildIndex(docs) {
    var idx = lunr(function () {
        ...
        this.field('t'); // title
        this.field('b'); // body
        // if I want to show a preview later but it increases the index size
...
function isIgnore(data) {
    return ((data.t.match(/^Tag: /) != null) || !data.p)
}
...
function main() {
    ...
    var docs = [];
    for (var i = 0; i < files.length; i++) {
        // don't scan /post/ articles which are just there for migration
        let html = readHtml(HTML_FOLDER, files[i], i)
        if (!isIgnore(html)) {
            docs.push(html);
        }
    }
